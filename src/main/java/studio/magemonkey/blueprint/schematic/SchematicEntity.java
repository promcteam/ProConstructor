...
public class SchematicEntity {
    ...
    private void removeUnwantedTags(Object nbtTagCompound) {
        ...
        for (Object passenger : NMS.getInstance()
                .getNMSProvider()
                .nbtTagCompound_getList(nbtTagCompound, "Passengers", 10)) {
    ...
    public Entity spawn(Location origin, int rotations) {
        ...
        NMSProvider nms      = NMS.getInstance().getNMSProvider();
        Object nmsEntity = nms.entityType_loadEntityRecursive(nbtCompound,
                Objects.requireNonNull(origin.getWorld()),
                (loadedEntity) -> {
                    nms.entity_absMoveTo(loadedEntity,
                            location.getX(),
                            location.getY(),
                            location.getZ(),
                            location.getYaw() + rotations * 90,
                            location.getPitch());
                    return loadedEntity;
                });
        if (nmsEntity != null) {
