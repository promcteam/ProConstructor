...
public class YAMLSchematic extends Schematic {
    ...
    public YAMLSchematic(Path path) {
        ...
        for (String key : keys) {
            ...
            this.tiers.add(new SchematicTier(new File(Blueprint.getInstance().config().getSchematicsFolder(),
                    Objects.requireNonNull(section.getString(SchematicTier.PATH), "Missing 'path' field")).toPath(),
                    section));
        }
    }
    ...
    @Override
    public @Nullable Vector getAbsolutePosition() {
        return this.tiers.get(Math.min(this.tier, this.getTotalTiers() - 1))
                .getAbsolutePosition();
    }
    ...
    public EmptyBuildBlock getBlockAt(int x, int y, int z) {
        return this.tiers.get(Math.min(this.getNextTier(),
                this.getTotalTiers() - 1)).getBlockAt(x, y, z);
    }

    @Override
    public Location offset(Location origin, double x, double y, double z, int emptyLayers, int rotation) {
        return this.tiers.get(Math.min(this.getNextTier(), this.getTotalTiers() - 1))
                .offset(origin, x, y, z, emptyLayers, rotation);
    }

    @Override
    public @NotNull Map<Material, Integer> getMaterials() {
        int tier = this.getNextTier();
        if (tier >= getTotalTiers()) {
            return new HashMap<>();
        }
