...
public class Util_1_18_R1 implements Util {
	...
	public static NBTBase fromNative(Tag foreign) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        if (foreign == null) {
            return null;
        }
        if (foreign instanceof CompoundTag) {
            NBTTagCompound tag = new NBTTagCompound();
            for (Entry<String, Tag> entry : ((CompoundTag) foreign)
                    .getValue().entrySet()) {
				Class[] partypes = new Class[2];
				partypes[0] = String.class;
				partypes[1] = NBTBase.class;
				tag.getClass().getMethod("a", partypes).invoke(tag,entry.getKey(), fromNative(entry.getValue()));
            }
            return tag;
            ...
            return NBTTagString.a(foreign.getValue().toString());
