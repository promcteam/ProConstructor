package com.promcteam.blueprint.hooks.citizens;

import com.promcteam.blueprint.Blueprint;
import com.promcteam.blueprint.Config;
...
import com.promcteam.blueprint.hooks.citizens.persistence.SchematicPersistenceLoader;
import com.promcteam.blueprint.menus.BuilderMenu;
import com.promcteam.blueprint.nms.NMS;
import com.promcteam.blueprint.schematic.Schematic;
import com.promcteam.blueprint.schematic.SchematicEntity;
...
public class BuilderTrait extends Trait implements Toggleable {
    ...
    @Persist("HoldItems")
    boolean            holdItems          = Blueprint.getInstance().config().isHoldItems();
    @Persist("RequireMaterials")
    boolean            requireMaterials   = Blueprint.getInstance().config().isRequireMaterials();
    ...
    public void setMoveTimeoutSeconds(double moveTimeoutSeconds) {
        this.moveTimeoutSeconds = Math.max(1, moveTimeoutSeconds);
    }
    ...
    public void setSchematic(@Nullable Schematic schematic) {
        ...
        } else if (this.requireMaterials && !this.getMissingMaterials().isEmpty()) {
    ...
    public void setRotation(int rotation) {
        this.rotation = Util.normalizeRotations(rotation);
    } // Fixme rotate actual block state
    ...
    public Map<Material, Integer> getMissingMaterials() {
        ...
        if (this.schematic == null || !this.requiresMaterials()) {
    ...
    public void onSpawn() {
        ...
        if (state == BuilderState.BUILDING || state == BuilderState.COLLECTING) {
            ...
            }.runTaskLater(Blueprint.getInstance(), 20);
    ...
    public void save(DataKey key) { // Can't do this in Persisters since they don't process null values
        key.setString("Schematic",
                this.schematic == null ? null
                        : new File(Blueprint.getInstance().config().getSchematicsFolder()).toPath()
                                .relativize(new File(this.schematic.getPath()).toPath())
                                .toString());
        key.setString("PersistentBuilding",
                this.persistentBuilding == null ? null : this.persistentBuilding.getUUID().toString());
    }
            ...
            if (heldItem.getType().isBlock() && !(heldItem.getType() == Material.AIR)) {
                ...
                if (needed > 0) {
                    ...
                    if (session != null && System.currentTimeMillis() < session + 5 * 1000) {
                        ...
                        this.materials.getHandle()
                                .put(material, this.materials.getHandle().getOrDefault(material, 0) + taking);
                        player.sendMessage(Blueprint.format(config.getSupplyTakenMessage(), this.npc,
                                this.schematic,
                                player,
                        ...
                        player.sendMessage(Blueprint.format(config.getSupplyNeedMessage(),
                                this.npc,
                                this.schematic,
                                player,
                                heldItem.getType().name().toLowerCase(),
                    ...
                    player.sendMessage(Blueprint.format(config.getSupplyDontNeedMessage(),
                            this.npc,
                            this.schematic,
                            player,
                            heldItem.getType().name().toLowerCase(),
    ...
    private boolean StartBuild(CommandSender player) {
        ...
        if (!silent) sender.sendMessage(Blueprint.format(
                Blueprint.getInstance().config().getStartedMessage(),
                npc,
                schematic, player, null,
                "0"));

        if (onStart != null) {
            String resp = Blueprint.runTask(onStart, npc);
            if (!silent) {
        ...
        Blueprint.denizenAction(npc, "Build Start");
        Blueprint.denizenAction(npc, "Build " + schematic.getDisplayName() + " Start");
    ...
    public void SetupNextBlock() {
        ...
        pending = Objects.requireNonNull(continueLoc.getWorld())
                .getBlockAt(schematic.offset(continueLoc, next.X, next.Y, next.Z, 0, this.rotation));

        if (next.getMat().equals(pending.getLocation().getBlock().getBlockData())) {
            SetupNextBlock();
        } else {
            if (npc.isSpawned()) {
                if ((npc.getEntity() instanceof org.bukkit.entity.HumanEntity
                        || npc.getEntity() instanceof org.bukkit.entity.Enderman) && this.holdItems) {
                    if ((npc.getEntity() instanceof org.bukkit.entity.HumanEntity) && this.holdItems) {
                        ((org.bukkit.entity.HumanEntity) npc.getEntity()).getInventory()
                                .setItemInHand(new ItemStack(next.getMat().getMaterial()));
                    } else if ((npc.getEntity() instanceof org.bukkit.entity.Enderman) && this.holdItems) {
                        ((org.bukkit.entity.Enderman) npc.getEntity()).setCarriedMaterial(new MaterialData(next.getMat()
                                .getMaterial()));
                    }
                }
            }
            ...
            }.runTaskLater(Blueprint.getInstance(), (long) (moveTimeoutSeconds * 20) + 1);
        }
    }
    ...
    public void SetupNextEntity() {
        ...
        pending = Objects.requireNonNull(continueLoc.getWorld())
                .getBlockAt(schematic.offset(continueLoc,
                        location.getX(),
                        location.getY(),
                        location.getZ(),
                        0,
        ...
        if (npc.isSpawned()) {
            if ((npc.getEntity() instanceof org.bukkit.entity.HumanEntity
                ...
                if ((npc.getEntity() instanceof org.bukkit.entity.HumanEntity) && this.holdItems) {
                    ((org.bukkit.entity.HumanEntity) npc.getEntity()).getInventory()
                            .setItemInHand(new ItemStack(Material.SHEEP_SPAWN_EGG));
                } else if ((npc.getEntity() instanceof org.bukkit.entity.Enderman) && this.holdItems) {
    ...
    public void CancelBuild() {
        if (onCancel != null) Blueprint.runTask(onCancel, npc);
        Blueprint.denizenAction(npc, "Build Cancel");
        if (schematic != null) Blueprint.denizenAction(npc, "Build " + schematic.getDisplayName() + " Cancel");
        stop();
    }
    ...
    public void CompleteBuild() {
        ...
        if (this.state == BuilderState.BUILDING) {
            ...
            if (!silent) sender.sendMessage(
                    Blueprint.format(Blueprint.getInstance().config().getCompleteMessage(),
                            npc,
                            schematic, sender,
                            null, "0"));

            if (onComplete != null) {
                String resp = Blueprint.runTask(onComplete, npc);
                if (!silent) {
            ...
            Blueprint.denizenAction(npc, "Build Complete");
            Blueprint.denizenAction(npc, "Build " + schematic.getDisplayName() + " Complete");
            if (this.schematic instanceof YAMLSchematic) {
                ...
                if (this.persistentBuilding == null || !this.persistentBuilding.getPath()
                        .equals(this.schematic.getPath())) {
                    this.persistentBuilding =
                            new PersistentBuilding(UUID.randomUUID(), yamlSchematic, tier, this.origin);
                } else {
    ...
    private void stop() {
        ...
        Plugin dynmap;
        if (stop && (dynmap = Bukkit.getServer().getPluginManager().getPlugin("dynmap")) != null
                && dynmap.isEnabled()) {
            org.dynmap.DynmapCommonAPI dyn = (DynmapCommonAPI) dynmap;
            Objects.requireNonNull(dyn)
                    .triggerRenderOfVolume(npc.getEntity().getWorld().getName(),
                            this.continueLoc.getBlockX() - schematic.getWidth() / 2,
                            this.continueLoc.getBlockY(),
                            this.continueLoc.getBlockZ() - schematic.getLength() / 2,
                            this.continueLoc.getBlockX() + schematic.getWidth() / 2,
                            this.continueLoc.getBlockY() + schematic.getHeight() / 2,
                            this.continueLoc.getBlockZ() + schematic.getLength() / 2);
        }
    ...
    public void PlaceNextBlock() throws
