...
public class BuildingRegistry {
    ...
    void load() {
        ...
        for (String key : this.config.getKeys(false)) {
            try {
                PersistentBuilding building =
                        new PersistentBuilding(key, Objects.requireNonNull(this.config.getConfigurationSection(key)));
                this.buildings.put(building.getUUID(), building);
            } catch (Exception e) {
                Blueprint.getInstance().getLogger().warning("Failed to load building '" + key + '\'');
                e.printStackTrace();
            }
        }
    }
    ...
    public PersistentBuilding getPersistentBuilding(Location location) {
        ...
        for (PersistentBuilding persistentBuilding : this.buildings.values()) {
            if (!persistentBuilding.getWorld().equals(location.getWorld())) {
                continue;
            }
            if (persistentBuilding.getBoundingBox().contains(vector)) {
