...
import com.promcteam.blueprint.API.StructureUtil;
...
public class StructureSubCommand extends AbstractCommand {
    ...
    public void execute(@NotNull CommandSender sender,
                        ...
                        @NotNull List<String> args) { // TODO bring back WorldEdit
        if (args.size() != 1) {
            sendUsage(sender);
            return;
        }
        ...
        player.sendMessage(ChatColor.GREEN + "Successfully saved structure " + ChatColor.WHITE + name + ChatColor.GREEN
                + " in schematics folder.");
    }
}
